...
export class InjectionManager {
  private platformSDKScript: string;
   ...
   * 注入平台 SDK
   */
  async injectPlatformSDK(webContents: WebContents): Promise<void> {
   ...
   * 注入 Adapter
   */
  async injectAdapter(webContents: WebContents, adapterScript: string): Promise<void> {
   ...
   */
  getAdapterForUrl(url: string): string | null {
    // 简单的 URL 匹配逻辑（后续可扩展）
  ...
  private getWhatsAppAdapter(): string {
    return `
      (function() {
  ...
  private getTelegramAdapter(): string {
    return `
      (function() {

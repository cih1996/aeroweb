...
export class BrowserClient {
  ...
  async req<T>(method: string, path: string, body?: unknown): Promise<T> {
    return new Promise((resolve, reject) => {
      ...
      const r = http.request({
        hostname: this.host, port: this.port, path: `/api${path}`, method,
        ...
        res.on('end', () => {
          ...
          } catch { reject(new Error(`Invalid response: ${result}`)); }
        });
      });
  ...
  newPage(i: string, url: string) { return this.req('POST', `/instance/${i}/page`, { url }); }

...
apps/desktop (Electron App)
  ├── 依赖 @poly-apps/browser-service
  │   └── 依赖 @poly-apps/shared
  │
  └── 依赖 @poly-apps/shared (直接使用类型和常量)
...
| 包名 | 类型 | 说明 | 主要导出 |
|------|------|------|----------|
| `@poly-apps/desktop` | App | Electron 桌面应用 | - |
| `@poly-apps/browser-service` | Package | 浏览器服务核心 | `BrowserService`, `InjectionManager`, `SessionManager` |
| `@poly-apps/shared` | Package | 共享类型和常量 | `Tab`, `Session`, `Profile`, `APP_IDS`, `APP_URLS` |
...
```
构建所有包：
- `@poly-apps/shared` → `dist/`
- `@poly-apps/browser-service` → `dist/`
- `@poly-apps/desktop` → `dist/main/`, `dist/preload/`, `dist/renderer/`
